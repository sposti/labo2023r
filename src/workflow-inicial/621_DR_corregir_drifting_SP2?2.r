...
AgregarVariables_IntraMes <- function(dataset) {
  ...
  dataset[, vm_status_SP := ifelse(is.na(Master_status), 0, Master_status)
          + ifelse(is.na(Visa_status), 0, Visa_status)] #status que empeora si la TC está en proceso de baja o dada de baja. El no tener no suma complejidad.
  ...
  dataset[, atm_ratio_m := rowSums(cbind(ifelse((matm + matm_other) == 0, 0, matm / (matm + matm_other))), na.rm = TRUE)] #
  ...
  dataset[, m_e_y_ratio := rowSums(cbind(ifelse(m_y_sp == 0, 0, m_e_sp / m_y_sp)), na.rm = TRUE)] #
  dataset[, c_e_y_ratio := rowSums(cbind(ifelse(c_y_sp == 0, 0, c_e_sp / c_y_sp)), na.rm = TRUE)] #
  dataset[, m_p_y_ratio := rowSums(cbind(ifelse(m_y_sp == 0, 0, m_p_sp / m_y_sp)), na.rm = TRUE)] #
  dataset[, m_p_e_ratio := rowSums(cbind(ifelse(m_e_sp == 0, 0, m_p_sp / m_e_sp)), na.rm = TRUE)] #
