...
PARAM <- list()
PARAM$experimento <- "HT7510_005sp"

PARAM$exp_input <- "TS7410_005sp"
...
PARAM$lgb_basicos <- list(
  ...
  verbosity = -100,
  max_depth = 9.0, #-1L, # -1 significa no limitar,  por ahora lo dejo fijo
  ...
  bagging_fraction = 0.66, # 0.0 < bagging_fraction <= 1.0
  pos_bagging_fraction = 0.66, # 0.0 < pos_bagging_fraction <= 1.0
  neg_bagging_fraction = 0.66, # 0.0 < neg_bagging_fraction <= 1.0
  is_unbalance = FALSE, #
...
PARAM$bo_lgb <- makeParamSet(
  ...
  makeIntegerParam("min_data_in_leaf", lower = 250L, upper = 50000L)
)
